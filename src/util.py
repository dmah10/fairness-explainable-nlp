...
def df_from_ferret_scores(male_evals, female_evals, lengths):
    male_scores = collect_ferret_scores(male_evals)
    female_scores = collect_ferret_scores(female_evals)
    scores = {"male": male_scores, "female": female_scores}
    ...
    results_df = pd.DataFrame(
        columns=["gender", "explainer", "metric", "score", "length"]
    )
    for gender, evals in scores.items():
        for i, (explainer, metrics) in enumerate(evals.items()):
